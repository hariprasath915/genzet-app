...
async function _syncPull() {
  ...
  try {
    ...
    if (typeof saveLibraryIDB === 'function') await saveLibraryIDB();
...
async function _syncPullCourses() {
  ...
  try {
    ...
    const localById = Object.fromEntries(
      (typeof engineeringCourses !== 'undefined' ? engineeringCourses : []).map(s => [s.id, s])
    );
    for (const s of cloud) localById[s.id] = s;
    if (typeof engineeringCourses !== 'undefined') engineeringCourses = Object.values(localById);
    if (typeof idbSet === 'function') await idbSet('eng_courses', engineeringCourses);
...
function _logout() {
  ...
  if (typeof saveLibraryIDB === 'function') saveLibraryIDB();
  if (typeof showFolders === 'function') showFolders();
  _showLandingWithModal();
}
