...
(function () {
  ...
  async function _syncAll() {
    ...
    if (r.ok) {
      ...
      _hydrateCourses(r.data.subjects || [], r.data.file_mode || []);
  ...
  function _hydrateCourses(subjects, fileModeRecords) {
    /**
     * subjects from /sync/all is already in engineeringCourses shape:
     * [{ id, name, description, cos: [...], units: [], file_mode_records: [...] }]
     *
     * fileModeRecords: flat array of file_mode rows from the new table.
     * We attach them to each subject as s.file_mode_records[].
     */
    // Build file_mode lookup by subject_id
    const fmBySubject = {};
    (fileModeRecords || []).forEach(rec => {
      const sid = rec.subject_id;
      if (sid) {
        if (!fmBySubject[sid]) fmBySubject[sid] = [];
        fmBySubject[sid].push(rec);
      }
    });
    ...
    subjects.forEach(s => {
      ...
      // Attach file_mode records (new) — replaces the old units[] array
      s.file_mode_records = fmBySubject[s.id] || (s.file_mode_records || []);
      // Keep units as empty array for backward safety (old code may check s.units)
      if (!Array.isArray(s.units)) s.units = [];
    });
    if (typeof engineeringCourses !== 'undefined') {
      engineeringCourses = subjects;
    }
    // Hydrate in-memory File Mode state
  ...
  // ════════════════════════════════════════════════════════════════════════
  // FILE MODE — file_mode table sync helpers
  // These are called by the new File Mode JS in index.html.
  // ════════════════════════════════════════════════════════════════════════

  /**
   * Create a new file_mode record.
   * @param {string} subjectId  - engineering_subjects.id (may be null)
   * @param {string} subjectName - display name
   * @param {string} unit        - unit display name (e.g. "Unit - 1")
   * @param {number} unitNumber  - e.g. 1, 2, 3
   * @param {string} unitType    - 'unit' | 'lesson'
   * @param {string[]} topics    - array of topic strings
   * @param {string} contents    - AI-generated HTML
   * @param {string} prompt      - prompt text
   * @param {string} title       - display title
   * @returns {Promise<object|null>} the created record, or null on failure
   */
  window.syncFileModeCreate = async function syncFileModeCreate(
    subjectId, subjectName, unit, unitNumber, unitType, topics, contents, prompt, title
  ) {
    const res = await _api('POST', '/sync/file-mode', {
      ...
      sort_order:  unitNumber  ? unitNumber - 1 : 0,
    });
    if (res.ok && res.data && res.data.record) {
      console.log('[SYNC] ✅ file_mode created:', res.data.record.id);
      return res.data.record;
    }
    console.warn('[SYNC] syncFileModeCreate failed:', res.error);
    return null;
  };

  /**
   * Update an existing file_mode record (partial patch).
   * @param {string} recordId
   * @param {object} patch  - { topics?, contents?, title?, prompt?, sort_order? }
   * @returns {Promise<object|null>} updated record, or null on failure
   */
  window.syncFileModeUpdate = async function syncFileModeUpdate(recordId, patch) {
    const res = await _api('PUT', '/sync/file-mode/' + encodeURIComponent(recordId), patch);
    if (res.ok && res.data && res.data.record) {
      console.log('[SYNC] ✅ file_mode updated:', recordId);
      return res.data.record;
    }
    console.warn('[SYNC] syncFileModeUpdate failed:', res.error);
    return null;
  };

  /**
   * Soft-delete a file_mode record.
   * @param {string} recordId
   * @returns {Promise<boolean>}
   */
  window.syncFileModeDelete = async function syncFileModeDelete(recordId) {
    const res = await _api('DELETE', '/sync/file-mode/' + encodeURIComponent(recordId));
    if (res.ok) {
      console.log('[SYNC] 🗑 file_mode deleted:', recordId);
      return true;
    }
    console.warn('[SYNC] syncFileModeDelete failed:', res.error);
    return false;
  };

  /**
   * List all file_mode records for a subject (or all for the user).
   * @param {string} [subjectId]  - optional filter
   * @returns {Promise<object[]>}  array of records, or [] on failure
   */
  window.syncFileModeList = async function syncFileModeList(subjectId) {
    const path = subjectId
      ? '/sync/file-mode/subject/' + encodeURIComponent(subjectId)
      : '/sync/file-mode';
    const res = await _api('GET', path);
    if (res.ok && res.data) return res.data.records || [];
    console.warn('[SYNC] syncFileModeList failed:', res.error);
    return [];
  };
